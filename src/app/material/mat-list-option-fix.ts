...
@Directive({
  ...
  export class MatListOptionFixDirective implements OnDestroy {
    private originalSelectedState: boolean;

    constructor(@Host() private matListOption: MatListOption) { 
        // Preserve the initial selection state when the directive is created.
        this.originalSelectedState = this.matListOption.selected;
    }
  
    ngOnDestroy() {
        // Restore the initial selection state before the option gets destroyed.
        // This prevents the selection list from updating its value when the option is removed.
        this.matListOption.selected = this.originalSelectedState;
    }
  }